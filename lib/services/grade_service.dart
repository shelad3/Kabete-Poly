...
class GradeService {
  ...
  Stream<List<GradeRecord>> getGradesForClass(
    String classId, {
    String? term,
    String? academicYear,
  }) {
  ...
  Future<Map<String, double>> getClassStandings(
    String classId, {
    String? term,
    String? academicYear,
  }) async {
    final grades = await getGradesForClass(
      classId,
      term: term,
    ...
    return studentTotals.map(
      (k, v) =>
          MapEntry(k, v.isEmpty ? 0 : v.reduce((a, b) => a + b) / v.length),
    );
  }
}
